				...
				<?php foreach ($arResult['ITEMS'] as $item): ?>
					...
					<div class="slider__item">
						<div class="slider__item-wrp">
							<img src="<?= $item['PREVIEW_PICTURE']['SRC'] ?>" alt="<?= $item['PREVIEW_TEXT'] ?>">
							<div class="slider__item-content-wrp">
								<h3><a href="<?= $item['DETAIL_PAGE_URL'] ?>"><?= $item['PREVIEW_TEXT'] ?></a></h3>
								<p><?= $price ?> ₽</p>
							</div>
						</div>
						<button onclick="window.location.href='<?= $item['DETAIL_PAGE_URL'] ?>'">купить срочно</button>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
